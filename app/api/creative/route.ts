...
async function fetchBatch(requests: { method: "GET"; relative_url: string }[], token: string): Promise<BatchItem[]> {
...
async function buildTokenByAccount(tokens: string[]): Promise<Record<string, string>> {
  const tokenByAccount: Record<string, string> = {};
  const results = await Promise.allSettled(tokens.map(async (token) => ({ token, accountIds: await fetchAccountIds(token) })));

  for (const result of results) {
    if (result.status !== "fulfilled") continue;
    for (const accountId of result.value.accountIds) {
      // Keep the first token for overlapping accounts so one ad is never requested twice.
...
async function processCreativeBatch(
  ...
  accountIds: string[],
  token: string
): Promise<Record<string, CreativeResult>> {
  ...
  await Promise.all(Object.entries(accountToHashes).flatMap(([accountId, hashes]) => {
  ...
  if (boostStories.length) {
    ...
    await Promise.all(boostStories.map(async (raw) => {
  ...
  const result: Record<string, CreativeResult> = {};
  for (const [adId, cid] of Object.entries(adToCreative)) {
    const raw = creativeRaw[cid];
    if (!raw) continue;
    result[adId] = {
      thumbnailUrl: (raw.imageHash && hashToUrl[raw.imageHash]) ||
...
export async function GET(req: NextRequest) {
  ...
  const results = await Promise.all(groups.flatMap((group) => {
    const requests: Promise<Record<string, CreativeResult>>[] = [];
    for (let i = 0; i < group.adIds.length; i += BATCH_SIZE) {
      requests.push(processCreativeBatch(
        group.adIds.slice(i, i + BATCH_SIZE),
        group.accountIds.slice(i, i + BATCH_SIZE),
        group.token
      ));
    }
    return requests;
