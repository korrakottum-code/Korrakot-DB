...
function MetricCard({ label, value, change, tone = "indigo" }: { label: string; value: string; change?: number | null; tone?: string }) {
  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-gradient-to-br from-slate-900 to-slate-950 p-4">
      ...
      {change != null && (
        <p className={`mt-1 flex items-center gap-1 text-xs ${positive ? "text-rose-300" : "text-emerald-300"}`}>
          {positive ? <ArrowUpRight className="h-3.5 w-3.5" /> : <ArrowDownRight className="h-3.5 w-3.5" />}
          {positive ? "+" : ""}{change.toFixed(1)}% เทียบช่วงก่อน
        </p>
      )}
    </div>
  );
...
export default function ManagementReport() {
  ...
  const branchSummaries = useMemo<BranchSummary[]>(() => {
    if (!report) return [];
    const map = new Map<string, { rows: AdInsight[]; objectiveKnown: boolean }>();
    for (const row of filteredInsights) {
      ...
      const current = map.get(name) || { rows: [], objectiveKnown: true };
    ...
    return [...map.entries()]
      .map(([name, group]) => {
        ...
        const result = decisionForBranch({ inbox, leads, objectiveKnown: group.objectiveKnown, cpi: inbox > 0 ? spend / inbox : null, cpl: leads > 0 ? spend / leads : null });
        return { name, spend, inbox, leads, impressions, cpi: inbox > 0 ? spend / inbox : null, cpl: leads > 0 ? spend / leads : null, share: totalSpend > 0 ? spend / totalSpend : 0, ...result };
      })
        ...
        {loading && !report ? (
          ...
          <>
              ...
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">
                    ...
                    {totalChange == null ? " ยังไม่มีช่วงก่อนหน้าให้เทียบ" : totalChange >= 0 ? ` โดยงบเพิ่ม ${totalChange.toFixed(1)}%` : ` โดยงบลด ${Math.abs(totalChange).toFixed(1)}%`}
            ...
            <section className="grid grid-cols-2 gap-3 md:grid-cols-4">
              <MetricCard label="ยอดใช้จ่าย" value={fmtMoney(report.totals.spend)} change={totalChange} tone="emerald" />
              <MetricCard label="Inbox" value={fmtNumber(report.totals.inbox)} change={pctChange(report.totals.inbox, report.previousTotals.inbox)} tone="purple" />
              ...
              <MetricCard label="Lead / CPL" value={`${fmtNumber(report.totals.leads)} / ${fmtMoney(report.totals.cpl)}`} change={pctChange(report.totals.leads, report.previousTotals.leads)} tone="pink" />
            </section>
                  ...
                  {branchSummaries.slice(0, 8).map((branch) => (
                    <div key={branch.name} className="rounded-xl border border-slate-800 bg-slate-950/60 p-3">
                      <div className="flex flex-wrap items-start justify-between gap-2">
                        <div><p className="font-medium text-white">{branch.name}</p><p className="mt-1 text-xs text-slate-500">Spend {fmtMoney(branch.spend)} · Share {(branch.share * 100).toFixed(1)}% · Sample {fmtNumber(branch.leads || branch.inbox)}</p></div>
            ...
            <section className="grid gap-5 lg:grid-cols-[1.2fr_1fr]">
              <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4 sm:p-5"><div className="flex items-center justify-between"><div><p className="text-xs font-semibold uppercase tracking-wider text-violet-300">Branch Scorecard</p><h2 className="mt-1 text-lg font-semibold">สาขาที่ใช้จ่ายสูงสุด</h2></div><Activity className="h-5 w-5 text-violet-300" /></div><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[680px] text-left text-xs"><thead className="border-b border-slate-800 text-slate-500"><tr><th className="px-2 py-2">สาขา</th><th className="px-2 py-2 text-right">Spend</th><th className="px-2 py-2 text-right">Share</th><th className="px-2 py-2 text-right">Inbox</th><th className="px-2 py-2 text-right">Lead</th><th className="px-2 py-2 text-right">CPI</th><th className="px-2 py-2 text-right">CPL</th></tr></thead><tbody>{branchSummaries.map((branch) => <tr key={branch.name} className="border-b border-slate-800/70"><td className="px-2 py-2 font-medium text-white">{branch.name}</td><td className="px-2 py-2 text-right text-emerald-300">{fmtMoney(branch.spend)}</td><td className="px-2 py-2 text-right text-slate-300">{(branch.share * 100).toFixed(1)}%</td><td className="px-2 py-2 text-right text-violet-300">{fmtNumber(branch.inbox)}</td><td className="px-2 py-2 text-right text-blue-300">{fmtNumber(branch.leads)}</td><td className="px-2 py-2 text-right text-amber-300">{fmtMoney(branch.cpi)}</td><td className="px-2 py-2 text-right text-pink-300">{fmtMoney(branch.cpl)}</td></tr>)}</tbody></table></div></div>
